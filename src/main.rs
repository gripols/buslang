...
impl<'a> Interpreter<'a> {
    ...
    fn execute(&mut self) {
        ...
        while i < self.source_code.len() {
            match self.source_code[i] {
                "ROUTE" => self.increment_data_pointer(),
                "102" => self.decrement_data_pointer(),
                "MARKHAM" => self.increment_value(),
                "ROAD" => self.decrement_value(),
                "SOUTHBOUND" => self.write_to_output(),
                "TOWARDS" => self.read_from_input(),
                "WARDEN" => {
                    if self.memory[self.data_pointer] == 0 {
                        let mut count = 1;
                        while count > 0 {
                            i += 1;
                            if self.source_code[i] == "WARDEN" {
                                count += 1;
                            } else if self.source_code[i] == "STATION" {
                                count -= 1;
                            }
                        }
                    }
                ...
                "STATION" => {
                    if self.memory[self.data_pointer] != 0 {
                        ...
                        while count > 0 {
                            i -= 1;
                            if self.source_code[i] == "STATION" {
                                count += 1;
                            } else if self.source_code[i] == "WARDEN" {
                                count -= 1;
                            }
                        }
                    }
                }
                "PRESTO" => self.debug_handler(),
                _ => eprintln!("unknown yap detected"),
            }
            i += 1;
        }
    }
    ...
    fn debug_handler(&self) {
        ...
        write!(
            &mut debug_output,
            "\x1b[1;34mdebug: \x1b[0mip \x1b[36m=\x1b[0m 
                                \x1b[33m0x{:x}\x1b[0m \x1b[90m(\x1b[0mprogram 
                                \x1b[36m+\x1b[0m \x1b[33m0x{:x}\x1b[0m\x1b[90m)
                                \x1b[36m;\x1b[0m dp\x1b[36m\x1b[0m \x1b[36m=\x1b[0m 
                                \x1b[33m0x{:x}\x1b[0m \x1b[90m(\x1b[0mmemory \x1b[36m+
                                \x1b[0m \x1b[33m0x{:x}\x1b[0m\x1b[90m)\x1b[0m\x1b[36m;\x1b[0m 
                                \x1b[36m*\x1b[0mdp \x1b[36m=\x1b[0m \x1b[33m{}\x1b[0m",
            data_pointer_addr, self.data_pointer, memory_addr, self.data_pointer, memory_value
        )
        .unwrap();
...
fn main() {
    ...
    let input_filename = args().nth(1).expect("Usage: bus-in-rust <source file>");
